...
**Hybrid RAG runs sparse (BM25) and dense (vector) retrieval in parallel, then fuses results to get the best of both worlds.**

**Why combine them:**
- **BM25:** Excellent at exact keyword matches, rare/technical terms
- **Vector search:** Excellent at semantic similarity, synonyms, paraphrasing
- **Together:** Complementary strengths cover each other's blind spots

**Fusion methods:** RRF (Reciprocal Rank Fusion) is simple and effective—merges ranked lists without calibrating scores.

**When to use:** When users mix semantic queries ("how does X work?") with keyword searches ("find ISO 27001"). Default choice for production.

**Trade-off:** Minimal—just runs two retrievers instead of one. The upside usually justifies the cost.
