...
flowchart TD
    subgraph "GraphRAG Pipeline"
        A1["Text Unitization"] --> B1["Entity/Relationship<br/>Extraction"]
        B1 --> C1["KG Construction"]
        C1 --> D1["Community Detection<br/>(Leiden Algorithm)"]
        D1 --> E1["Community<br/>Summarization"]
    end

    subgraph "LightRAG Pipeline"
        A2["Text Unitization"] --> B2["Entity/Relationship<br/>Extraction"]
        B2 --> C2["KG Construction +<br/>Chunk Linking"]
        C2 --> D2["Keyword Index<br/>Building"]
    end
...
flowchart LR
    ...
    KW --> LL["Low-Level<br/>Retrieval"]
    KW --> HL["High-Level<br/>Retrieval"]

    LL --> E["Specific Entities +<br/>Direct Relationships"]
    HL --> T["Broader Topics +<br/>Cross-Entity Aggregation"]

    E --> M["Merge & Rank"]
    T --> M
    M --> C["Retrieve Linked<br/>Text Chunks"]
