...
```mermaid
flowchart TD
    D["Document Corpus"] --> LLM["LLM<br/>(Open IE Extraction)"]
    LLM --> T["Entity Triples<br/>(subject, predicate, object)"]
    T --> KG["Knowledge Graph<br/>Construction"]
    KG --> IDX["Node Embeddings +<br/>Passage Links"]

    subgraph "Pattern Separation"
        P1["Passage A: 'Einstein developed<br/>the theory of relativity'"] --> N1["Einstein"]
        P1 --> N2["Theory of Relativity"]
        P2["Passage B: 'Einstein won the<br/>Nobel Prize in 1921'"] --> N1
        P2 --> N3["Nobel Prize"]
        P2 --> N4["1921"]
    end
...
```mermaid
flowchart LR
    Q["Query"] --> EXT["Extract Query<br/>Entities"]
    EXT --> MATCH["Match to KG Nodes<br/>(Seed Nodes)"]
    MATCH --> PPR["Run Personalized<br/>PageRank"]
    PPR --> RANK["Rank All Nodes<br/>by PPR Score"]
    RANK --> TOP["Select Top-K<br/>Nodes"]
    TOP --> PASS["Retrieve Linked<br/>Passages"]
...
flowchart TD
    subgraph "Standard PageRank"
        direction LR
        A1["Node A<br/>Start: 1/N"] ---|"equal"| B1["Node B<br/>Start: 1/N"]
        B1 ---|"equal"| C1["Node C<br/>Start: 1/N"]
        C1 ---|"equal"| D1["Node D<br/>Start: 1/N"]
    end

    subgraph "Personalized PageRank"
        direction LR
        A2["Seed Node A<br/>Start: 0.5"] -->|"propagate"| B2["Node B<br/>Start: 0"]
        A2S["Seed Node C<br/>Start: 0.5"] -->|"propagate"| B2
        B2 -->|"propagate"| D2["Node D<br/>Start: 0"]
    end
...
```mermaid
graph TD
    E["Einstein<br/>(Seed: 0.5)"] -->|"developed"| QM["Quantum<br/>Mechanics"]
    E -->|"developed"| REL["Theory of<br/>Relativity"]
    E -->|"worked_at"| IAS["Institute for<br/>Advanced Study"]
    QM -->|"foundation_of"| QC["Quantum<br/>Computing<br/>(Seed: 0.5)"]
    QM -->|"includes"| ENT["Quantum<br/>Entanglement"]
    ENT -->|"enables"| QC
    QC -->|"uses"| QG["Quantum<br/>Gates"]
    QC -->|"pioneered_by"| FEY["Feynman"]
    FEY -->|"studied_at"| IAS
    REL -->|"relates_to"| GPS["GPS<br/>Satellites"]
